...
struct pilha
{
    ...
    float *vet;//o vetor vai armazenar os dados da pilha
};
...
void pilha_push(Pilha *p, float v){
    if (p->n == N)
    {
        printf("Numero maximo da pilha alcancado\n");
         p->vet = (float*)realloc(p->vet,(N*2)*sizeof(float));
    }

    p->vet[p->n] = v;//significa que esta acessando o topo da pilha
...
float pilha_pop(Pilha *p){
    ...
    v = p->vet[p->n-1];//v vai receber o valor da ultima posicao ou seja o topo da pilha
